...
int main(int argc, char *argv[])
{
  ...
  printf(1, "\n This program tests the correctness of your lab#1 and lab#2\n");
  ...
  if (atoi(argv[1]) == 1)
	...
	PScheduler();
  else if (atoi(argv[1]) == 4) // Aging Option
	Aging();
  else 
   printf(1, "\nType \"Test 1\" to test exit and wait, \"Test 2\" to test waitpid, \"Test 3\" to test the priority scheduler, and \"Test 4\" to test aging.\n");
...
int PScheduler(void){
  ...
  printf(1, "\n  Step 1: Testing the priority scheduler and setpriority(int priority)) system call:\n");
...
int Aging(void) {
  int pid = fork();
  int i = 0;
  if(pid == 0) { // Set child to lower priority
    sleep(1);
    setpriority(3);
    for(i = 0; i < 100; i++) {
      printf(1, "Child\n");
    }
  } else { // Set parent to higher priority
    sleep(1);
    setpriority(0);
    for(i = 0; i < 100; i++) {
      printf(1, "Parent\n");
